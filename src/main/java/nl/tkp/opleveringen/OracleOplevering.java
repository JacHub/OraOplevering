    ...
    private void haalDependencies(List<String> regels, String upperObjectName, String oracleType) {
            ...
            try (CallableStatement stmt = connection.prepareCall("{? = call chk_vsn_pck.dependencies_unknown_owner(?,?)}")) {
                ...
                if (wholes.size()>0) {
                    ...
                    regels.add("         for gen in ( select * from gen_object_versies where deelapplicatie = l_deelapplicatie and objectnaam = p_name and objecttype = p_type order by aanmaakdatum desc )");
                    regels.add("         loop");
                    regels.add("            if gen.versienummer < l_versie then");
                    regels.add("               raise_application_error (-20001, 'Lager versienummer gevonden bij dependency: [' || p_owner || '][' || p_name || '][' || p_type || ']. Gevonden versie: [' || gen.versienummer || '], verwacht: [' || l_versie || ']. **** De oplevering wordt afgebroken! ****');");
                    regels.add("            end if;");
                    regels.add("            exit;");
                    regels.add("         end loop;");
                    regels.add("      end if;");
